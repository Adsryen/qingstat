...
const reservedDoubleFields = Array.from({ length: 9 }, (_, index) =>
    field({
        slot: `double${index + 12}` as const,
        logicalName: `reservedDouble${index + 12}`,
        valueType: "number",
        status: "reserved",
        source: "reserved",
        cardinality: "none",
...
export const METRICS_V1_DOUBLE_FIELDS = [
    ...
    field({
        slot: ColumnMappings.ttfbMs,
        logicalName: "ttfbMs",
        valueType: "number",
        status: "used",
        source: "tracker perf sample (navigation responseStart), 50ms buckets, ~10% sample",
        cardinality: "numeric",
        maxLength: null,
        nullable: true,
        privacy: "derived",
        queryRole: "sampled TTFB ms; 0 = not sampled",
        compatibility: "locked double9; pageviews only when sampled",
        rollup: "never",
    }),
    field({
        slot: ColumnMappings.lcpMs,
        logicalName: "lcpMs",
        valueType: "number",
        status: "used",
        source: "tracker perf sample (loadEventEnd coarse), 50ms buckets, ~10% sample",
        cardinality: "numeric",
        maxLength: null,
        nullable: true,
        privacy: "derived",
        queryRole: "sampled load timing ms; 0 = not sampled",
        compatibility: "locked double10; not true LCP observer yet",
        rollup: "never",
    }),
    field({
        slot: ColumnMappings.errorEvent,
        logicalName: "errorEvent",
        valueType: "number",
        status: "used",
        source: "tracker window.onerror / unhandledrejection samples",
        cardinality: "numeric",
        maxLength: null,
        nullable: false,
        privacy: "derived",
        queryRole: "1 = JS error sample; 2 = custom event; pageview queries exclude errorEvent!=0",
        compatibility: "locked double11; 0=pageview, 1=error sample, 2=custom event; historical rows treated as 0",
        rollup: "never",
    }),
